...
4) 你最多可以调用工具 {tool_times} 次；当你已经具备足够信息时，必须输出 Final，不要无意义地重复调用工具。
5) 如果你已经可以给出最终答案，必须输出：
 Final: <你的回答应当完备而严谨，并严格遵守引用格式。
         正文中凡是用到了某条 Observation 的信息，必须在该句末尾按引用次序标注对应编号，例如：[1][2]，此顺序应该与你的回答引用一致。
...
<forbidden>
硬性禁止（违反即视为错误输出）：
1) 你获得的回答必须严格根据 Observation 内容生成，禁止凭空编造事实。
2) 绝对禁止在你的输出中包含以 “Observation:” 开头的内容；Observation 只能由外部工具执行结果注入。
3) 当你输出了 Action/Action Input 时，本轮输出必须立刻结束，不允许继续输出 Observation 或 Final。
4) 当你输出 Final 时，本轮输出中不允许再出现 Action/Action Input/Observation。
5) 当用户问题输入不为法律相关问题时，你需要直接回答 “我只是法律问答系统，换个问题试试吧”
</forbidden>
...
def render_prompt(*, tools: str, question: str, history: str = "", tool_times: int = 5) -> str:
