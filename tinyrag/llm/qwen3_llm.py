...
class qwen3_llm(BaseLLM):
    ...
    def generate(self, content: str) -> str:
        system_guard = (
            ...
            "2) 绝对禁止输出 Observation（包括以“Observation:”开头的任何文本）；Observation 只能由外部工具执行结果注入。\n"
            "3) 若你需要调用工具，本轮只输出 Thought/Action/Action Input 三段，并在 Action Input 后立刻结束，不要输出 Final。\n"
            "4) 只有当你不需要调用任何工具时才输出 Final；输出 Final 时不得包含 Action/Action Input/Observation。\n"
